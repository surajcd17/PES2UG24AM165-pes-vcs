...
int index_load(Index *index) {
    ...
    while (index->count < MAX_INDEX_ENTRIES &&
           fscanf(f, "%o %64s %ld %zu %s", 
                  &index->entries[index->count].mode, hex,
                  &index->entries[index->count].mtime_sec,
                  &index->entries[index->count].size, path) == 5) {
        
        hex_to_hash(hex, &index->entries[index->count].id);
        strncpy(index->entries[index->count].path, path, MAX_PATH_LEN - 1);
        index->count++;
    }
    fclose(f);
    return 0;
